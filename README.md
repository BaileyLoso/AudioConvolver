...
## Download
### [Latest release](https://github.com/BaileyLoso/AudioConvolver/releases/latest)
- [Windows x64](https://github.com/BaileyLoso/AudioConvolver/releases/latest/download/AudioConvolver-windows-x64.zip)
...
## How it works
This project uses audio *convolution* to combine two signals into a new signal: 
...
* Convolving with the *overlap-add method*
using the scipy *oaconvolve* method for longer signals or 
highly mismatched lengths.

## Requirements files
This repo uses two dependency lists:
* `requirements.txt` — **runtime dependencies** needed to run the app
* `requirements-dev.txt` — **developer tooling** used to build an executable
...
```
 ### Building the app
```aiignore
pyinstaller --clean --noconfirm AudioConvolver.spec
```

## Requirements
  Tested with Python 3.12. Newer versions may work but are not currently tested.

### NOTE: 
- AudioConvolver is currently only supported on Windows 11, but future versions may support macOS and Linux.
